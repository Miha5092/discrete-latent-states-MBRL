...
zs = []
z = representation_net.sample(torch.tensor(env.state(), dtype=torch.float32, device=device).unsqueeze(0).permute(0, 3, 1, 2))
zs.append(z)
reconstructions = []
...
def on_key_press(event):
    ...
    if action == 404:
        ...
        z, predicted_reward, predicted_done = dynamics_net.sample(dynamics_net_input)
        # z is the next latent state by dynamics prediction
        z_prime = representation_net.sample(torch.tensor(env.state(), dtype=torch.float32, device=device).unsqueeze(0).permute(0, 3, 1, 2))
        print( ((z-z_prime)**2).mean() )

        # reward = dynamics_net.reward_net(z)
        # done_logits = dynamics_net.done_net(z)
