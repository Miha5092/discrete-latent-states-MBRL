...
regularizations = [0.0, 0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]
for regularization in regularizations:
    ...
    with open("cluster/template.sbatch", "r") as f:
        ...
        """
        command = f"python3 learn_model.py --unroll_steps=1 --num_epochs=20 --image_prediction_loss=MSE --out_channels=128 --data_path=dqn_training_seed_0/experience_data_full.npz --learning_rate=0.00025 --exp_name=test_reg_{regularization} --seed=0 --weight_decay={regularization}"
        ...
        save template
        """
        sbatch_name = f"cluster/sbatches/model_learning_regularization_{regularization}.sbatch"
        ...
        sync to delftblue
        """
        os.system(f"rsync -trau {sbatch_name} delftblue:learn_models_minatar/cluster/sbatches/model_learning_regularization_{regularization}.sbatch")

        """
        execute the sbatch file on delftblue
        """
        os.system(f"ssh delftblue 'cd learn_models_minatar; sbatch cluster/sbatches/model_learning_regularization_{regularization}.sbatch'")
