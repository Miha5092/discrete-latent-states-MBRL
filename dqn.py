...
#
################################################################################################################
def world_dynamics(t, replay_start_size, num_actions, s, env, policy_net, no_epsilon: bool = False):
...
def dqn(
    ...
    if load_model is not None:
        policy_net = torch.load(load_model)
        target_off = True
    else:
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            s_prime, action, reward, is_terminated = world_dynamics(t, replay_start_size, num_actions, s, env, policy_net, no_epsilon)
...
def main():
    ...
    parser.add_argument("--sticky_prob", "-sp", type=float, default=0.0)
    parser.add_argument("--load_model", "-lm", type=str)
    parser.add_argument("--display", "-d", action="store_true")
    parser.add_argument("--no_epsilon", action="store_true")
    ...
    # no sticky actions
    env = Environment(args.game, sticky_action_prob=args.sticky_prob, use_minimal_action_set=True)

    print('Cuda available?: ' + str(torch.cuda.is_available()), flush=True)
    dqn(env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha, args.seed, args.load_model, args.display, args.no_epsilon)
